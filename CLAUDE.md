...
- [ ] Budget-chooser presets: rework `BudgetQueryScreen` to offer two one-click preset buttons (30 min, 60 min) plus a third "Eigene Zeit…" option that reveals the existing text-input field. Avoids the typing requirement for the common case while keeping arbitrary values reachable.
- [ ] Player name in history log: add a `player` column to `HistoryEntry` (read from `Minecraft.getInstance().getUser().getName()` or equivalent at attempt time). Useful for shared machines where multiple kids play under different Mojang accounts. Update `HistoryLogger.HEADER` and column widths.
- [ ] Change default budget from 60 min → 30 min: edit `DEFAULT_MINUTES` in `BudgetQueryScreen` (one-line tweak). Subsumed by the budget-chooser-presets TODO if that lands first — the preset buttons would supersede the text field's prefill.
